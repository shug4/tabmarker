{/* <script> */}
    ...
    //       if (t.cols == null) t.cols = state.cols;
    //       if (t.rows == null) t.rows = state.rows;
    //     });
    // if (!state.tabs || !state.tabs.length) state.tabs = [{id: uid(), name: 'Home', cols: state.cols, rows: state.rows }];
...
//   }
//   try {
//     if (state.bgImage) localStorage.setItem('bm_bg', state.bgImage);
//     else localStorage.removeItem('bm_bg');
//   } catch (e) {
//     toast('⚠ 背景画像が大きすぎて保存できませんでした');
//   }
// }
...
function load() {
  try {
    ...
    //   } else {
    //     el.style.background = 'linear-gradient(135deg, #fbdbea 0%, #e8d1f3 40%, #e8c0e5 70%, #cfe7f6 100%)';
    //   }
    // }
...
function renderGrid() {
  ...
  grid.style.gridTemplateRows = `repeat(${rows}, 1fr)`;
    ...
    if (state.settingsOn) {
        el.textContent = '＋';
        el.addEventListener('click', () => openModal(null, c, r));
    ...
    function openModal(bm, col = 0, row = 0) {
        modalTarget = bm;
    ...
    // const img = document.createElement('img');
    // img.src = src;
    //   img.onerror = () => {p.innerHTML = '<span>faviconが見つかりません</span>'; };
    // p.appendChild(img);
    ...
    if (bm) {bm.url = fullUrl; bm.name = name; bm.favicon = favicon; }
      } else {
    ...
    // document.getElementById('settings-panel').style.display = state.settingsOn ? 'block' : 'none';
    // save(); render();
    // });
    document.getElementById('settings-btn').addEventListener('click', () => {
        state.settingsOn = !state.settingsOn;
...
//   document.documentElement.style.setProperty('--text', isBlack ? 'rgba(0,0,0,0.85)' : 'rgba(255,255,255,0.92)');
//   document.documentElement.style.setProperty('--text-dim', isBlack ? 'rgba(0,0,0,0.45)' : 'rgba(255,255,255,0.55)');
//   document.documentElement.style.setProperty('--glass-border', isBlack ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.25)');
...
//   const THRESHOLD = 60;   // スワイプと判定する最小px
//   const ANGLE = 30;       // 横方向と判定する最大角度(度)
...
//     if (angle > ANGLE && angle < 180 - ANGLE) return;
...
//   const THRESHOLD = 60; // スワイプ判定距離
//   const ANGLE = 30;    // 横方向とみなす最大角度
...
//     // 左スワイプ → 次タブ
//     if (dx < 0 && idx < state.tabs.length - 1) {
//       state.activeTab = state.tabs[idx + 1].id;
//     }
